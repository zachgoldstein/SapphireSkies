package org.flixel
{
	...
	public class FlxG
	{
		 ...
		 */
		static public var _cache:Object;
		 ...
		 * Creates a new seamless looping sound object from an embedded <code>Class</code> object.
		 * Use this for looping MP3s to eliminate the silence at the beginning of the file.
		 * Note: Not currently integrated with code to allow dynamic pitch changes.
		 * 
		 * @param	EmbeddedSound	The sound you want to play.
		 * @param	Volume			How loud to play it (0 to 1).
		 * @param	Looped			Whether or not to loop this sound.
		 ...
		 * @return	A <code>FlxSound</code> object.
		 */
		static public function playLoopFix(EmbeddedSound:Class,Volume:Number=1.0,Looped:Boolean=false, totalSamples:int = 0):FlxSound
		{
			var sl:uint = sounds.length;
			for(var i:uint = 0; i < sl; i++)
				if(!(sounds[i] as FlxSound).active)
			...
			if(sounds[i] == null)
				sounds[i] = new FlxSound();
			var s:FlxSound = sounds[i];
			s.loadEmbeddedLoopFix(EmbeddedSound,Looped,totalSamples);
			s.volume = Volume;
			s.play();
			return s;
		}
