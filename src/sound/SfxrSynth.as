package sound
{
	...
	public class SfxrSynth extends EventDispatcher
	{
		...
		public var _params:SfxrParams = new SfxrParams;	// Params instance
		
		public var _sound:Sound;							// Sound instance used to play the sound
		public var _channel:SoundChannel;					// SoundChannel instance of playing Sound
		
		public var _mutation:Boolean;						// If the current sound playing or caching is a mutation
		
		public var _cachedWave:ByteArray;					// Cached wave data from a cacheSound() call
		public var _cachingNormal:Boolean;					// If the synth is caching a normal sound
		
		public var _cachingMutation:int;					// Current caching ID
		public var _cachedMutation:ByteArray;				// Current caching wave data for mutation
		public var _cachedMutations:Vector.<ByteArray>;	// Cached mutated wave data
		public var _cachedMutationsNum:uint;				// Number of cached mutations
		public var _cachedMutationAmount:Number;			// Amount to mutate during cache
		
		public var _cachingAsync:Boolean;					// If the synth is currently caching asynchronously
		public var _cacheTimePerFrame:uint;				// Maximum time allowed per frame to cache sound asynchronously
		public var _cachedCallback:Function;				// Function to call when finished caching asynchronously
		public var _cacheTicker:Shape;						// Shape used for enterFrame event
		
		public var _waveData:ByteArray;					// Full wave, read out in chuncks by the onSampleData method
		public var _waveDataPos:uint;						// Current position in the waveData
		public var _waveDataLength:uint;					// Number of bytes in the waveData
		public var _waveDataBytes:uint;					// Number of bytes to write to the soundcard
		
		public var _original:SfxrParams;					// Copied properties for mutation base
		...
		public function play():void
		{
			...
			if (!_sound) (_sound = new Sound()).addEventListener(SampleDataEvent.SAMPLE_DATA, onSampleData,false,0,true);
		...
		public function testSampleData(e:SampleDataEvent):void
		{
			trace(ptest(onSampleData, [e], "Test1"));
		}
		...
		{
			var sampleProcess:SoundEffectProcess = new SoundEffectProcess(this,e);
			sampleProcess.run();
...
/*			
		 ...
		 */
		public function synthWave(buffer:ByteArray, length:uint, waveData:Boolean = false, sampleRate:uint = 44100, bitDepth:uint = 16):Boolean
		{
			_finished = false;
			...
			for(var i:uint = 0; i < length; i++)
			{
				...
				for(var j:int = 0; j < 8; j++)
				{
					...
					switch(_waveType)
					{
						...
						case 3: // Noise
						{
							if(uint(_phase * 32 / int(_periodTemp)) < 32){
