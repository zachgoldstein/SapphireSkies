package org.flixel
{
	...
	public class FlxTilemap extends FlxObject
	{
		...
		/**
		 * Returns an array of the tiles within a certain area of the tilemap.
		 * 
		 ...
		 */
		public function getTilesInArea(X:Number,Y:Number,Width:Number,Height:Number):Array
		{
			var numTilesWide:uint = (X+Width)/_tileWidth - (X/_tileWidth);
			var numTilesHigh:uint = (Y+Height)/_tileHeight - (Y/_tileHeight);
			
			var tilesInArea:Array = new Array();
			for (var i:int = 0 ; i < numTilesHigh +1 ; i++){
				for (var j:int = 0 ; j < numTilesWide +1 ; j++){
					tilesInArea.push( getTile( (X/_tileWidth)+j, (Y/_tileHeight)+i ) ); 
				}
			}
			return tilesInArea;
		}
		...
		/**
		 * Sets all the tiles in a certain region to null (0). Returns the number of tiles cleared. 
		 * @param X
		 * @param Y
		 * @param Width
		 * @param Height
		 ...
		 * 
		 */
		public function clearTilesInArea(X:Number,Y:Number,Width:Number,Height:Number):Number
		{
			
			var numTilesWide:Number = Math.ceil( (X+Width)/_tileWidth - (X/_tileWidth) );
			var numTilesHigh:Number = Math.ceil( (Y+Height)/_tileHeight - (Y/_tileHeight) );

			//testing
/*			var testObj:FlxSprite = new FlxSprite();
			testObj.fixed = true;			
			testObj.createGraphic(Width,Height);
			testObj.alpha = .5;w
			testObj.x = Math.floor(X/_tileWidth)*_tileWidth;
			testObj.y = Math.floor(Y/_tileHeight)*_tileHeight;
			(FlxG.state as PlayState).add(testObj);
*/			
			var tilesCleared:Number = 0;
			for (var i:int = 0 ; i < numTilesHigh ; i++){
				for (var j:int = 0 ; j < numTilesWide ; j++){
					if(getTile((X/_tileWidth)+j,(Y/_tileHeight)+i) != 0 ){
						tilesCleared++;
					}
					setTile((X/_tileWidth)+j, (Y/_tileHeight)+i, 0);
				}
			}
		...
		public function clearTilesInCircle(centreX:Number,centreY:Number,radius:Number):Number
		{
			var numTilesWide:uint = Math.round(radius*2/_tileWidth);
			var numTilesHigh:uint = Math.round(radius*2/_tileHeight);
			
			var tilesCleared:Number = 0;
			for (var i:int = 0 ; i < numTilesHigh ; i++){
				for (var j:int = 0 ; j < numTilesWide ; j++){
					//get point on block
					var blockPoint:Point = new Point(centreX-radius + j*_tileWidth,centreY-radius + i*_tileHeight);
					var distance:Number = Math.sqrt( (blockPoint.x-centreX)*(blockPoint.x-centreX) + (blockPoint.y-centreY)*(blockPoint.y-centreY) );
					var testPoint:Number = ((centreX-radius)/_tileWidth)+j;
					if(distance <= radius){
						if(getTile(((centreX-radius)/_tileWidth)+j,((centreY-radius)/_tileHeight)+i) != 0 ){
							tilesCleared++;
							setTile(((centreX-radius)/_tileWidth)+j,((centreY-radius)/_tileHeight)+i, 0);
						}
					}
				}
			}
