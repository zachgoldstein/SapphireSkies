...
package com
{
	...
	public class BaseLevel
	{
		...
		public var specialTileLocations:Dictionary = new Dictionary();
		
		// This group contains all the tilemaps specified to use collisions.
		public var specialTilemaps:FlxGroup = new FlxGroup;
		public var decorationTilemaps:FlxGroup = new FlxGroup;
		...
		public function addTileMapWithString( mapString:String, imageClass:Class, x:Number, y:Number, tileWidth:uint, tileHeight:uint, scrollX:Number, scrollY:Number, hits:Boolean, collideIdx:uint, drawIdx:uint, properties:Array, onAddCallback:Function = null ):FlxTilemap
		{
			var map:FlxTilemap = new FlxTilemap;
			map.collideIndex = collideIdx;
			map.drawIndex = drawIdx;
			map.loadMap( mapString, imageClass, tileWidth, tileHeight );
			map.x = x;
			map.y = y;
			map.scrollFactor.x = scrollX;
			map.scrollFactor.y = scrollY;
		...
		public function addSpriteToLayer(passedObj:FlxObject, type:Class, layer:FlxGroup, x:Number, y:Number, angle:Number, flipped:Boolean = false, scaleX:Number = 1, scaleY:Number = 1, properties:Array = null, onAddCallback:Function = null):FlxObject
		{
			if( passedObj == null )
				passedObj = new type(x, y);
			if(passedObj is FlxSprite){
				var obj:FlxSprite = passedObj as FlxSprite;
				obj.x += obj.offset.x;
				obj.y += obj.offset.y;
				obj.angle = angle;
				...
				return obj;
			} else if(passedObj is FlxGroup){
				layer.add(passedObj,true);
				callbackNewData(passedObj, onAddCallback, layer, properties, false);
				return passedObj;
			}
			return null
		}
